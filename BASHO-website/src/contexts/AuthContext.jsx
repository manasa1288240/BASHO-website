...
export const AuthProvider = ({ children }) => {
  ...
  // Mock sendOtp — in real app this should call backend
  const sendOtp = async (contact) => {
    const requestId = "req_" + Date.now();
    // For development store a predictable OTP
    const otp = "1234";
  ...
  const verifyOtp = async ({ requestId, otp }) => {
    ...
    // Create or return user — auto-signup behavior
    ...
    // Notify other parts of the app (cart) that login occurred so they can merge local state
    try {
